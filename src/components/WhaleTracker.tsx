...
export function WhaleTracker({ tokenAddress }: { tokenAddress: string }) {
  ...
  useEffect(() => {
    console.log('[WhaleTracker] Component mounted with token:', tokenAddress);
    return () => console.log('[WhaleTracker] Component unmounted');
  }, []);
  
  useEffect(() => {
    console.log('[WhaleTracker] Token address changed to:', tokenAddress);
  }, [tokenAddress]);
  
  useEffect(() => {
    console.log('[WhaleTracker] Loading state:', loading);
  }, [loading]);
  
  useEffect(() => {
    if (error) console.log('[WhaleTracker] Error state:', error);
  }, [error]);
  
  useEffect(() => {
    console.log('[WhaleTracker] Holders updated:', holders.length);
  }, [holders]);
  
  useEffect(() => {
    console.log('[WhaleTracker] Transfers updated:', transfers.length);
  }, [transfers]);
  
  useEffect(() => {
    console.log('[WhaleTracker] Token metadata updated:', tokenMetadata);
  }, [tokenMetadata]);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      {tokenMetadata ? (
        <div className="mb-6 flex items-center gap-4 p-4 border rounded-lg bg-card">
          <div className="flex-shrink-0">
            <img 
              src={tokenMetadata.logo || 'https://raw.githubusercontent.com/ethereum-lists/chains/master/_data/icons/base.png'} 
              alt={tokenMetadata.name} 
              className="w-16 h-16 rounded-full object-cover border-2 border-border"
              onError={(e) => {
                // Fallback to a generic token icon if image fails to load
                (e.target as HTMLImageElement).src = 'https://raw.githubusercontent.com/ethereum-lists/chains/master/_data/icons/base.png';
              }}
            />
          </div>
          <div>
            ...
            <div className="text-sm text-muted-foreground mt-1">
              <div className="flex items-center gap-2">
                <span className="font-mono">{truncateAddress(tokenAddress)}</span>
                <button 
              ...
              {totalSupply && (
                <div className="mt-1">
                  Total Supply: {formatTokenAmount(totalSupply, tokenMetadata.decimals)} {tokenMetadata.symbol}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="mb-6">
        ...
        {followed && (
          <span className="ml-4 text-sm">Following: <span className="font-mono">{truncateAddress(followed)}</span></span>
        )}
      </div>
      
      {loading && (
        <>
          {(() => { console.log('[WhaleTracker] Showing loading state'); return null; })()}
      ...
      {error && (
        <>
          {(() => { console.log('[WhaleTracker] Showing error state:', error); return null; })()}
          <div className="py-4 px-6 rounded bg-red-50 border border-red-200 text-red-800">
            ...
            <details className="text-xs mt-2">
              ...
              <pre className="mt-2 p-2 bg-gray-100 rounded overflow-auto max-h-40">{error}</pre>
            </details>
          </div>
        </>
      )}
      ...
      {!loading && !error && holders.length === 0 && (
        <div className="py-8 text-center text-muted-foreground">
          No holder data found. This could be a new token or the contract doesn't implement the ERC20 standard.
        </div>
      )}
            ...
            <table className="w-full border rounded mb-4">
              <thead>
                <tr className="bg-muted">
                  ...
                  {totalSupply && <th className="px-2 py-1 text-left">% of Supply</th>}
              ...
              <tbody>
                {holders.map((h, i) => (
                  <tr key={h.address} className="border-b hover:bg-muted/50">
                    ...
                    <td className="px-2 py-1 font-mono">{truncateAddress(h.address)}</td>
                    ...
                    {totalSupply && <td className="px-2 py-1">{h.percentage?.toFixed(2)}%</td>}
                    <td className="px-2 py-1">
              ...
              <tbody>
                {transfers.slice(0, 50).map((t, i) => (
                  <tr key={t.txHash + i} className="border-b hover:bg-muted/50">
                    <td className="px-2 py-1 font-mono">{truncateAddress(t.from)}</td>
                    <td className="px-2 py-1 font-mono">{truncateAddress(t.to)}</td>
                    <td className="px-2 py-1">
                      {tokenMetadata 
                    ...
                    <td className="px-2 py-1">{formatTimestamp(t.timestamp)}</td>
                    <td className="px-2 py-1">
