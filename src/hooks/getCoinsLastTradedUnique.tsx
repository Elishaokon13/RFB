...
// Core types for last traded unique coin data matching Zora Protocol structure
interface LastTradedUniqueCoin {
  id?: string;
  name?: string;
  description?: string;
  address?: string;
...
export const useGetCoinsLastTradedUnique = (params: ExploreQueryOptions = {}) => {
  const { count = 20, after } = params;
  
  return useQuery<QueryResponse>({
    queryKey: ["coins", "last-traded-unique", "zora", { count, after }],
    queryFn: async () => {
      ...
      return {
        coins: tokens as LastTradedUniqueCoin[],
        pagination: {
...
export const getHolderActivityLevel = (uniqueHolders?: number): string => {
  if (!uniqueHolders) return "bg-gray-100 text-gray-800";
  
  if (uniqueHolders >= 10000) return "bg-purple-100 text-purple-800"; // Very high activity
  if (uniqueHolders >= 1000) return "bg-green-100 text-green-800"; // High activity
  if (uniqueHolders >= 100) return "bg-blue-100 text-blue-800"; // Moderate activity
  if (uniqueHolders >= 10) return "bg-yellow-100 text-yellow-800"; // Low activity
  return "bg-gray-100 text-gray-800"; // Very low activity
};
...
export const getCommunityEngagement = (uniqueHolders?: number, volume24h?: string): string => {
  if (!uniqueHolders || !volume24h) return "bg-gray-100 text-gray-800";
  
  const volumeValue = parseFloat(volume24h);
  if (isNaN(volumeValue)) return "bg-gray-100 text-gray-800";
  
  const engagementScore = uniqueHolders * Math.log10(volumeValue);
  
  if (engagementScore >= 100000) return "bg-purple-100 text-purple-800"; // Very engaged
  if (engagementScore >= 10000) return "bg-green-100 text-green-800"; // Engaged
  if (engagementScore >= 1000) return "bg-blue-100 text-blue-800"; // Moderate
  if (engagementScore >= 100) return "bg-yellow-100 text-yellow-800"; // Low
  return "bg-gray-100 text-gray-800"; // Very low
};

export type { LastTradedUniqueCoin, ExploreQueryOptions, QueryResponse }; 