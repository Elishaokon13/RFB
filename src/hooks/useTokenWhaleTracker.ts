...
export function useTokenWhaleTracker({
  ...
  useEffect(() => {
    ...
    async function fetchTransfers() {
      ...
      console.log('[useTokenWhaleTracker] Hook initialized with:', {
        tokenAddress,
        hasCustomABI: !!abi,
        startBlock
      });
      ...
      const sanitizedTokenAddress = sanitizeEthereumAddress(tokenAddress, 'useTokenWhaleTracker');
      ...
      console.log('[useTokenWhaleTracker] Address sanitization:', {
        original: tokenAddress,
        sanitized: sanitizedTokenAddress
      });
      
      // Infura API key for Base network
      const INFURA_API_KEY = import.meta.env.VITE_INFURA_API_KEY || 'e38fa8e8a1804ce1aa6f37cd51d62fb5';
      ...
      while (!provider && providerIndex < RPC_URLS.length) {
        try {
          console.log(`[useTokenWhaleTracker] Trying provider ${providerIndex + 1}/${RPC_URLS.length}: ${RPC_URLS[providerIndex].split('/')[2]}`);
          provider = new ethers.JsonRpcProvider(RPC_URLS[providerIndex]);
          await provider.getBlockNumber(); // Test connection
          console.log(`[useTokenWhaleTracker] Successfully connected to ${RPC_URLS[providerIndex].split('/')[2]}`);
        } catch (error) {
          console.warn(`[useTokenWhaleTracker] Failed to connect to ${RPC_URLS[providerIndex].split('/')[2]}:`, error);
          provider = null;
          providerIndex++;
        }
      }
      ...
      try {
        console.log('[useTokenWhaleTracker] Creating contract instance');
        ...
        console.log('[useTokenWhaleTracker] Fetching token metadata');
        
        try {
          // Try to get token name
          metadata.name = await contract.name();
          console.log('[useTokenWhaleTracker] Token name:', metadata.name);
        } catch (err) {
          console.warn('[useTokenWhaleTracker] Could not fetch token name:', err);
        }
        ...
        try {
          ...
          console.warn('[useTokenWhaleTracker] Could not fetch token symbol:', err);
        }
        
        try {
          // Try to get token decimals
          metadata.decimals = await contract.decimals();
          console.log('[useTokenWhaleTracker] Token decimals:', metadata.decimals);
        } catch (err) {
          console.warn('[useTokenWhaleTracker] Could not fetch decimals, using default (18):', err);
        }
        ...
        try {
          ...
          console.log('[useTokenWhaleTracker] Total supply:', metadata.totalSupply.toString());
        } catch (err) {
          console.warn('[useTokenWhaleTracker] Could not fetch total supply:', err);
        }
        ...
        try {
          ...
          try {
            console.log('[useTokenWhaleTracker] Trying to fetch token info from Coingecko');
            const coingeckoResponse = await fetch(`https://api.coingecko.com/api/v3/coins/base/contract/${sanitizedTokenAddress}`);
            ...
            if (coingeckoResponse.ok) {
              ...
              if (tokenData && tokenData.image && tokenData.image.large) {
                ...
                console.log('[useTokenWhaleTracker] Found logo from Coingecko:', metadata.logo);
              }
            } else {
              console.log('[useTokenWhaleTracker] Coingecko API returned status:', coingeckoResponse.status);
              ...
              if (metadata.symbol) {
                ...
                if (searchResponse.ok) {
                  ...
                  if (searchData && searchData.coins && searchData.coins.length > 0) {
                    ...
                    const matchingCoin = searchData.coins.find((coin: CoingeckoToken) => 
                      coin.symbol.toLowerCase() === metadata.symbol.toLowerCase()
                    );
                    
                    if (matchingCoin && matchingCoin.image) {
                      metadata.logo = matchingCoin.image;
                      console.log('[useTokenWhaleTracker] Found logo from Coingecko search:', metadata.logo);
                    }
                  }
                }
              }
            ...
            console.warn('[useTokenWhaleTracker] Error fetching from Coingecko:', coingeckoError);
          }
          ...
          if (metadata.logo === baseExplorerLogoUrl) {
            ...
            console.log('[useTokenWhaleTracker] Using Trustwallet logo URL as fallback:', metadata.logo);
          }
          ...
          console.warn('[useTokenWhaleTracker] Error fetching token logo:', err);
          setTokenMetadata(metadata);
        }
        ...
        const BLOCKS_TO_SEARCH = startBlock > 0 
        ...
        const initialFromBlock = Math.max(startBlock, latestBlock - BLOCKS_TO_SEARCH);
        
        console.log('[useTokenWhaleTracker] Will search from block', initialFromBlock, 'to', latestBlock);
        console.log('[useTokenWhaleTracker] Total block range:', latestBlock - initialFromBlock, 'blocks');
        ...
        const chunks = [];
        for (let start = initialFromBlock; start < latestBlock; start += MAX_BLOCK_RANGE) {
          const end = Math.min(start + MAX_BLOCK_RANGE - 1, latestBlock);
          chunks.push({ fromBlock: start, toBlock: end });
        }
        
        console.log(`[useTokenWhaleTracker] Splitting query into ${chunks.length} chunks of ≤${MAX_BLOCK_RANGE} blocks`);
        
        // Process each chunk with delay between requests
        for (let i = 0; i < chunks.length && !cancelled; i++) {
          const { fromBlock, toBlock } = chunks[i];
          console.log(`[useTokenWhaleTracker] Querying chunk ${i+1}/${chunks.length}: blocks ${fromBlock}-${toBlock} (range: ${toBlock - fromBlock + 1})`);
          
          try {
            // Add delay between requests to avoid rate limits
            if (i > 0) {
              console.log(`[useTokenWhaleTracker] Waiting 1 second before next request...`);
              await sleep(1000);
            }
            ...
            console.log(`[useTokenWhaleTracker] Found ${chunkEvents.length} events in chunk ${i+1}`);
            allEvents = [...allEvents, ...chunkEvents];
            
            // Update progress
            setProgress(Math.floor((i + 1) / chunks.length * 100));
          } catch (error) {
            console.error(`[useTokenWhaleTracker] Error in chunk ${i+1}:`, error);
            // Continue with other chunks even if one fails
          }
        }
        
        console.log('[useTokenWhaleTracker] Total events found across all chunks:', allEvents.length);
        
        // Aggregate balances
        const balances = new Map<string, bigint>();
        const txs: WhaleTransferEvent[] = [];
        
        console.log('[useTokenWhaleTracker] Processing events to calculate balances');
        ...
        console.log('[useTokenWhaleTracker] Processed', txs.length, 'transfers');
        console.log('[useTokenWhaleTracker] Found', balances.size, 'unique addresses');
        ...
        const holderArr: WhaleHolder[] = Array.from(balances.entries())
          .map(([address, balance]) => ({ 
            address, 
            balance,
            percentage: metadata.totalSupply > 0n ? Number((balance * 10000n / metadata.totalSupply)) / 100 : undefined
          }))
          .filter(h => h.balance > 0n)
          .sort((a, b) => (b.balance > a.balance ? 1 : b.balance < a.balance ? -1 : 0));
        
        console.log('[useTokenWhaleTracker] Filtered to', holderArr.length, 'holders with positive balance');
        ...
        console.log('[useTokenWhaleTracker] Getting timestamps for most recent 50 transactions');
    ...
    return () => {
      cancelled = true;
      console.log('[useTokenWhaleTracker] Cleanup - cancelled any pending operations');
    };
