...
export const useWalletTokens = () => {
  ...
  const getTokenBalances = async (
    ...
    try {
      ...
      if (balanceData.error) {
        throw new Error(
          balanceData.error.message || "Failed to fetch token balances"
        );
      }

      const tokenBalances: TokenBalance[] =
        balanceData.result.tokenBalances || [];
  ...
  const fetchTokens = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch wallet tokens"
      );
      setTokens([]);
    } finally {
      setLoading(false);
