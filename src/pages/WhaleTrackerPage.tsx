...
export default function WhaleTrackerPage() {
  ...
  useEffect(() => {
    console.log('[WhaleTrackerPage] Component mounted');
    return () => console.log('[WhaleTrackerPage] Component unmounted');
  }, []);

  useEffect(() => {
    console.log('[WhaleTrackerPage] Token address input changed:', tokenAddress);
    // Clear error when input changes
    if (inputError) setInputError(null);
  }, [tokenAddress, inputError]);

  useEffect(() => {
    console.log('[WhaleTrackerPage] Submitted address changed:', submittedAddress);
  }, [submittedAddress]);

  // Handle example token click
  const handleExampleClick = (token: { name: string; address: string }) => {
    console.log('[WhaleTrackerPage] Example token clicked:', token.name, token.address);
  ...
  const handleInputFocus = () => {
    console.log('[WhaleTrackerPage] Input focused');
  };

  const handleInputBlur = () => {
    console.log('[WhaleTrackerPage] Input blurred');
  };

  // Button hover handler
  const handleButtonHover = () => {
    console.log('[WhaleTrackerPage] Track button hover');
  };

  // Submit handler
  const handleSubmit = () => {
    console.log('[WhaleTrackerPage] Track button clicked');
    ...
    console.log('[WhaleTrackerPage] Submitting address:', sanitizedAddress);
        ...
        <main className="flex-1 p-8 pb-24">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-8 p-6 border rounded-lg bg-card shadow-sm">
              <h2 className="text-xl font-semibold mb-4">Track Token Holders & Transfers</h2>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <input
                      id="token-address"
                      className={`w-full border rounded-md px-4 py-2 pr-10 focus:outline-none focus:ring-2 focus:ring-primary/50 ${
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-sm font-medium mb-2">Stablecoins & Major Tokens</h3>
                ...
                <div>
                  <h3 className="text-sm font-medium mb-2">Popular Base Tokens</h3>
                  <div className="flex flex-wrap gap-2">
            ...
            {/* Log rendering and wrap in IIFE to avoid React node issues */}
            {(() => {
              console.log('[WhaleTrackerPage] Rendering WhaleTracker component for:', submittedAddress);
              return null;
            })()}
            ...
            {!submittedAddress && (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="mt-4">Enter a token address above or select one of the example tokens to start tracking whales and top holders.</p>
              </div>
            )}
          </div>
        </main>
